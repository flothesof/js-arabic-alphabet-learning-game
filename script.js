...
function updateExerciseProgressBar() {
  ...
  progressBar.innerHTML =
    "Question : " +
    (quizzData["correct"] + quizzData["incorrect"]) +
    "/" +
    quizzData["total"] +
    " | Correctes : " +
    quizzData["correct"] +
...
function generateNewRecognitionQuestion() {
  ...
  if (document.getElementById("game-area").hasAttributes("answer")) {
    ...
    var possibletaskLetters = taskLetters.filter(function (value) {
      return value != previousValue;
    });
    randomAnswer = possibletaskLetters[randInt(possibletaskLetters.length)];
    randomLetter = LETTER_MAPPING[randomAnswer];
  } else {
    // generate any new random letter the first time we start the game
    var randomAnswer = taskLetters[randInt(taskLetters.length)];
...
function validateRecognitionAnswer(e) {
  ...
  if (quizzData["correct"] + quizzData["incorrect"] < quizzData["total"]) {
    ...
    if (document.getElementById("game-area").answer == caller.innerHTML) {
      ...
      sketcher.strokes(strokesCorrectAnswer).clear(true).animate.strokes();
    } else {
      ...
      sketcher.strokes(strokesIncorrectAnswer).clear(true).animate.strokes();
    }
        ...
        header.appendChild(
          document.createTextNode(
            "Exercice " +
              ...
              "." +
              (parseInt(exerciseIndex) + 1) +
              " : " +
              " Reconnaître les lettres " +
              tasktaskLetters.join(" - ")
          )
        );
...
function runLevel(chapterIndex, exerciseIndex) {
  ...
  if (taskType == "recognition") {
    ...
    document.getElementById("game-area").currentLevel = [
      chapterIndex,
      exerciseIndex,
    ];
    ...
    document.getElementById("game-area").quizzData = {
      total: 32,
      correct: 0,
      incorrect: 0,
    };
...
function updateStarRating(chapterIndex, exerciseIndex, correct) {
  ...
  let previousStars = document
    .getElementById(`score-chapter${chapterIndex}-exercise${exerciseIndex}`)
    .innerHTML.split("Score : ")[1];
  let previousScore = REVERSE_STAR_MAPPING[previousStars];
  if (previousScore < score) {
    document.getElementById(
      `score-chapter${chapterIndex}-exercise${exerciseIndex}`
    ).innerHTML = "Score : " + STAR_MAPPING[score];
  }
...
function generateNewDrawingQuestion() {
  ...
  if (document.getElementById("game-area").hasAttributes("answer")) {
    ...
    let possibletaskLetters = taskLetters.filter(function (value) {
      return value != previousValue;
    });
    randomAnswer = possibletaskLetters[randInt(possibletaskLetters.length)];
  } else {
    // generate any new random letter the first time we start the game
    randomAnswer = taskLetters[randInt(taskLetters.length)];
  }
  // saving the new answer for later
  document.getElementById("game-area").answer = randomAnswer;

  // writing new drawing question
  document.getElementById("drawing-text-area").innerHTML =
    "La lettre à dessiner est : " + randomAnswer;
  updateExerciseProgressBar();
...
function showDrawingAnswer(e) {
  ...
  drawLetterOnCanvas(
    LETTER_MAPPING[document.getElementById("game-area").answer],
    false
  );
...
function updateChapterProgress() {
  ...
  for (let chapterIndex in LEVELS) {
    ...
    if (previousChapterCompleted && chapterIndex !== "0") {
      document.getElementById(
        `details-chapter${parseInt(chapterIndex) - 1}`
      ).open = false;
    }
    ...
    document.getElementById(
      ...
      (aboveThreeStars / exercicesInChapter) * 100
    )}%`;

    if (aboveFiveStars === exercicesInChapter) {
      document.getElementById(`chapter-progress-${chapterIndex}`).innerHTML +=
        " PERFECT";
    }
...
function saveProgress() {
  ...
  for (let chapterIndex in LEVELS) {
    for (let exerciseIndex in LEVELS[chapterIndex]) {
      ...
      savedValues.push([[id, currentScore]]);
    }
  }
